...
export const CultureInfo: React.FC<CultureInfoProps> = ({ cultureService, propertyService, groundService, bombService, irrigationSystemService }) => {
  ...
  const { data: dataCulture, isLoading: isLoadingCulture, refetch } = useQuery({
    queryKey: ["cultures"], 
  ...
  const grounds = !groundQuery.isLoading && groundQuery.data.data.map(item => {
    return {
      name: item.tipo_solo,
      id: item.id_solo
    }
  ...
  const bombs = !bombQuery.isLoading && bombQuery.data.data.map(item => {
    return {
      name: item.modelo,
  ...
  const systems = !systemsQuery.isLoading && systemsQuery.data.data.map(item => {
    return {
      name: item.nome,
      ...
      />
        <ScrollView style={{ paddingHorizontal: 16, flex: 1 }}>
          ...
          <S.Content>
            ...
            />
            <S.CategoryTitle>{strings.CultureInfo.category.hotalicas}</S.CategoryTitle>
            ...
            <S.CategoryTitle>{strings.CultureInfo.category.frutíferas}</S.CategoryTitle>
            {dataCulture && dataCulture.data.map(item => {
            ...
            <S.CategoryTitle>{strings.CultureInfo.category.grãosecereais}</S.CategoryTitle>
            {dataCulture && dataCulture.data.map(item => {
            ...
            <S.CategoryTitle>{strings.CultureInfo.category.raiz}</S.CategoryTitle>
            {dataCulture && dataCulture.data.map(item => {
            ...
            <S.CategoryTitle>{strings.CultureInfo.category.pastagem}</S.CategoryTitle>
            {dataCulture && dataCulture.data.map(item => {
            ...
            <S.CategoryTitle>{strings.CultureInfo.category.capineira}</S.CategoryTitle>
            {dataCulture && dataCulture.data.map(item => {
